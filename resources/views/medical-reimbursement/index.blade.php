        ...
        <div class="content-wrapper">
            <div class="content-body">
                <p class="d-flex align-items-center">
                    ...
                    <a href="{{ url('medical-reimbursement') }}" class="this-page-link">/ Reimbursement / Medical Reimbursement</a>
                </p>
                <div class="card">
                    <div class="card-body">
                        <span class="medical-tracker-reimbursement">Medical Tracker Reimbursement</span>
                        <div class="card">
                                    ...
                                    <div class="card-body">
                                        <div class="row">
                                            <div class="col-lg-3 col-md-6 d-flex">
                                                <img src="{{ url('img/icons/monetization-on-blue.svg') }}" alt="monetization-on-blue">
                                                <div class="statistic">
                                                    <div id="reimbursement-balance"></div>
                                                    <div>Reimbursement Balance</div>
                                                </div>
                                            </div>
                                            <div class="col-lg-3 col-md-6 d-flex">
                                                <img src="{{ url('img/icons/more-time-orange.svg') }}" alt="more-time-orange">
                                                <div class="statistic">
                        ...
                        <section id="medical-reimbursement-pending-status">
                            <div class="d-flex align-items-center justify-content-between">
                                ...
                                <button type="button" id="request-medical-reimbursement" class="btn btn-primary" data-toggle="modal" data-target="#modal-request-medical-reimbursement">+ Request Reimbursement</button>
                            </div>
                        ...
                        <section id="medical-reimbursement-history">
                            <h4>Medical Reimbursement History</h4>
                            <div class="table-responsive table-rounded-outline text-nowrap">
                                <table id="medical-reimbursement-history-datatables" class="table table-borderless">
                                    <thead>
                                        <tr>
                                            <th>No</th>
                                            <th>Type of Reimbursement</th>
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="modal-title">Request Medical Reimbursement</h4>
                ...
                <div class="modal-body">
                    <form id="form-data" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <select name="category" id="category" class="form-control" onchange="checkSelectShow(this)" required>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="amount" id="amount" class="form-control" placeholder="Rp." required>
...
@section('page_script')
    <script>
        const statistics = (year) => {
            $.ajax({
                url: "{{ url('medical-reimbursement/statistic') }}" + '/' + year,
                method: 'GET',
                dataType: 'json',
                success: (result) => {
                    const {
                    ...
                    $('#reimbursement-balance').html(`Rp${reimbursementBalance.toLocaleString('id-ID')}`);
        ...
        const pendingStatusData = () => {
            $.ajax({
                url: "{{ url('medical-reimbursement/data') }}",
                method: 'GET',
                dataType: 'json',
                success: (result) => {
                    let tr = '';
                    ...
                    if (result.data.length == 0) {
                        ...
                        result.data.forEach((element, index) => {
                            tr += `
                                <tr class="data-row-${element.medical_reimbursement_id}">
                                    <td>${index + 1}</td>
                                    <td>${element.reimbursement_type}</td>
                                    <td>${element.reimbursement_date}</td>
                                    ...
                                    <td class="text-center">
                                        <button type="button" id="pending-status-details" data-id="${element.medical_reimbursement_id}" class="btn btn-outline-info round pending-status-details" onclick="location.href='{{ url('medical-reimbursement/details/${element.medical_reimbursement_id}') }}'">Details</button>
                                    </td>
                                    <td class="text-center">
                                        <button type="button" id="pending-status-cancel" data-id="${element.medical_reimbursement_id}" class="btn btn-outline-danger round pending-status-cancel">Cancel</button>
                                    </td>
                                </tr>
                            `;
                        });
        ...
        $(document).ready(function() {
            ...
            const dataTablesAjax = (type, status) => {
                const dt_ajax_table = $("#medical-reimbursement-history-datatables");
                ...
                if (dt_ajax_table.length) {
                    const dt_ajax = dt_ajax_table.dataTable({
                        ...
                        ajax: `medical-reimbursement/list/${type}/${status}`,
                        ordering: false,
                        language: {
                            paginate: {
                                previous: '&nbsp;',
                        ...
                        columns: [{
                            ...
                            },
                            {
                                data: (data) => {
                                    if (data.category == 0) {
                                        ...
                                        if (data.outpatient_category == 0) {
                                            return 'Outpatient - Laboratory Examinations';
                                        } else if (data.outpatient_category == 1) {
                                            return 'Outpatient - Doctor Consultation';
                                        } else {
                                            return 'Outpatient - Glasses Replacement';
                                        }
                                    }
                                },
                                name: 'Type of Reimbursement'
                            },
                            {
                                data: (data) => data.reimbursement_date.split('-').reverse().join('/'),
                            ...
                            {
                                data: (data) => {
                                    if (data.total_reimburse == null) {
                                        return '-';
                                    } else {
                                        return 'Rp' + data.total_reimburse.toLocaleString('id-ID');
                                    }
                                },
                ...
                $("#leave-history-export").html('<div><img src="{{ url('./img/icons/open-in-new.svg') }}"><a href="{{ url('medical-reimbursement/export') }}" style="color: inherit;">Export</a></div><img src="{{ url('./img/icons/angle-down.svg') }}" id="angle-down">');
            }
                    ...
                    $.ajax({
                        ...
                        success: (data) => {
                            if (data.error) {
                                ...
                                let fields = ['category', 'outpatient_type', 'amount', 'attachment', 'notes'];
                                ...
                                setTimeout(() => {
                                    ...
                                    $('#count-glasses-reimbursement').load(location.href + ' #count-glasses-reimbursement');
                                }, 0);
                    ...
                    $.ajax({
                        ...
                        success: (result) => {
                            if (result.error) {
                                ...
                                setTimeout(() => {
                                    pendingStatusData();
                                    $('#medical-reimbursement-history-datatables').DataTable().ajax.reload();
                                    $('#count-glasses-reimbursement').load(location.href + ' #count-glasses-reimbursement');
                                }, 0);
