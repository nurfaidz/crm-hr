                    ...
                    <div class="bs-stepper horizontal-wizard-example">
                        ...
                        <div class="bs-stepper-header">
                            ...
                            <div class="step" style="margin-right: 0">
                                <button type="button"
                                    ...
                                    <span class="bs-stepper-label">
                                        <span class="bs-stepper-title first-bst active-bs-stepper-title">Finance
                                            Approval</span>
                                    </span>
                                </button>
                            </div>
                        ...
                        <div class="bs-stepper-content">
                            ...
                            <div id="leave-details">
                                <div class="row">
                                    ...
                                    <div class="col-6" id="leave-details-notes">
                                        <form id="attachment_form" enctype="multipart/form-data" novalidate>
                                            @csrf
                                            @method('PUT')
                                            <p>Payment Evidence</p>
                                            <div class="form-group">
                                                <input type="file" name="payment_evidence" id="payment_evidence"
                                                    placeholder="Choose files" class="form-control"
                                                    onchange="getAttachmentData(this)" multiple required>
                                                <div class="invalid-feedback payment_evidence_error"></div>
                                            </div>
                                        </form>
                                        <div class="form-group">
                                            <div class="invalid-feedback payment_evidence_error"></div>
                        ...
                        <div class="card-footer">
                            <div class="text-right pr-1 leave-details-buttons">
                                ...
                                @can('role_or_permission:Super Admin|medical_reimbursement_approval.manage')
                                    <a href="{{ url('approval/medical-reimbursement') }}/{{ $medicalReimbursement->medical_reimbursement_id }}/details-human-resource"
                                        class="btn btn-primary mr-1">Previous</a>
                                @endcan

                                @can('medical_reimbursement_approval.reject_finance')
                                ...
                                @endcan
                                @can('medical_reimbursement_approval.approve_finance')
                                    <button type="button" onclick=ApproveAction($(this))
                                        data-id="{{ $medicalReimbursement->medical_reimbursement_id }}"
                                        class="btn btn-success leave-details-approve" id="approve">Approve</button>
                                @else
    ...
    <script>
        ...
        const ApproveAction = (e) => {
            swal.fire({
                ...
                console.log($('#attachment_form').serialize());
                // console.log($('#attachment').val());
                if (result.value) {
                    const id = e.attr('data-id');
                    const base_url = "{{ url('approval/medical-reimbursement') }}";

                    const formAttachment = document.querySelector('#attachment_form');
                    if (formAttachment) {
                        const request = new FormData(formAttachment);
                        ...
                        if (input.files.length > 0) {
                            for (let i = 0; i < input.files.length; i++) {
                                formData.append(`payment_evidence[${i}]`, input.files[i]);
                            }
                        } else {
                            formData.append('payment_evidence', request.get('payment_evidence'));
                        }

                        fetch(`${base_url}/${id}/approve-finance?_method=PUT`, {
                                method: 'POST',
                                body: formData,
                            })
                            ...
                            .then(result => {
                                if (result.error) {
                                    Swal.fire({
                                        type: "error",
                                        title: 'Oops...',
                                        text: result.error.payment_evidence,
                                        confirmButtonClass: 'btn btn-primary',
                                    });
                                } else {
                                    setTimeout(function() {
            ...
            Array.prototype.filter.call($('#form_data'), (form) => {
                $('#submit').unbind().on('click', (event) => {
                    ...
                    swal.fire({
                        ...
                        if (result.value) {
                            const base_url =
                                "{{ url('approval/medical-reimbursement') }}";
                            const id = e.attr('data-id');
                            $.ajax({
                                url: `${base_url}/${id}/reject-finance`,
                                method: 'PUT',
        ...
        const getAttachmentData = (attachment) => {
            ...
            let files = document.getElementById('payment_evidence').files;
